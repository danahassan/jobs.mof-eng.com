  ...
  await Promise.allSettled(
    SHELL_ROUTES.map(async route => {
      try {
        ...
        if (res && res.ok && res.status < 400) await cache.put(route, res.clone());
      } catch (_) {}
    })
  );
}
async function offlineFallback(req) {
  ...
  if (req.mode === 'navigate') {
    const d = await caches.match('/dashboard'); if (d) return d;
    const h = await caches.match('/');          if (h) return h;
    const l = await caches.match('/login');     if (l) return l;
  }
...
self.addEventListener('fetch', e => {
  ...
  e.respondWith(
    fetch(req).then(res => {
      ...
      return res;
    }).catch(() => caches.match(req, {ignoreSearch:true}).then(c => c || new Response('', {status:408})))
  );
});
...
self.addEventListener('push', event => {
  ...
  const options = {
    body, icon: data.icon || ICON, badge: data.badge || BADGE,
    image: data.image || undefined, tag, renotify: true,
    requireInteraction: !!data.requireInteraction, silent: false,
    timestamp: Date.now(), vibrate: [120, 60, 120],
    ...
    clients.matchAll({type:'window', includeUncontrolled:true}).then(cs => {
      for (const c of cs) {
        try {
          ...
          if (u.pathname + u.search === dest || c.url === dest) return c.focus();
        } catch (_) {}
      }
  ...
  event.waitUntil((async () => {
    try {
      ...
      const sub = await self.registration.pushManager.subscribe({
        userVisibleOnly: true, applicationServerKey: conv(publicKey),
      });
